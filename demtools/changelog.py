#!/usr/bin/env python
"""
Changelog content shown by `demtools -cl` / `demtools --change-log`.

Kept in sync manually with CHANGELOG.md at the repository root (the same way
`__version__` is kept in sync with pyproject.toml) so the content ships
inside the installed package instead of relying on a repo-root file that
isn't included in the built wheel.
"""
